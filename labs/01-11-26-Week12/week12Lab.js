 ...
 *
 * ↓ YOUR CODE HERE ↓ */
$.get(API_URL).then(data => console.log(data))
...
$('#studentForm').on('submit', function (e) {
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({
            ...
            researchAssignment: $('#researchInput').val()
        })
    });
});

// displays the list of students on the webpage
/* function loadStudents() {
    ...
    $.get(API_URL).then(data => {
        data.forEach(student => {
            $('#studentList').append(
                `<li>
                <strong>${student.fullName}</strong> - ${student.researchAssignment}
            </li>`
            );
        });
    });
...
function deleteUserByName() {
    ...
    if (!fullName) {
        alert('Enter a full name to delete');
        return;
    }

    $.get(API_URL).then(data => {
        const student = data.find(s => s.fullName === fullName);
...
function updateUser() {
    const id = $('#updateIdInput').val();
    const fullName = $('#updateFullNameInput').val();
    const researchAssignment = $('#updateResearchInput').val();
